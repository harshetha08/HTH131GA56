...
def create_pdf_report(dataset_name, question, answer, explanation):
    buffer = BytesIO()
    ...
    story.append(
        Paragraph("Your data. Your questions. Instant insights.", body_style)
    )
    story.append(Spacer(1, 20))

    story.append(Paragraph("DATA ANALYSIS REPORT", heading_style))
    story.append(Spacer(1, 10))

    story.append(
        Paragraph(f"<b>Dataset:</b> {dataset_name}", body_style)
    )
    story.append(Spacer(1, 10))

    story.append(
        Paragraph(f"<b>Question:</b> {question}", body_style)
    )
    ...
    story.append(
        Paragraph(str(answer), body_style)
    )
    story.append(Spacer(1, 15))

    story.append(Paragraph("How it was calculated", heading_style))
    story.append(Spacer(1, 8))
...
def record_success(message):
    st.session_state["pdf_answer"] = str(message)
    st.success(message)


def record_info(message):
    st.session_state["pdf_explanation"] = str(message)
    st.info(message)
# --------------------------------------------------
# PAGE SETTINGS
# --------------------------------------------------
...
def choose_chart_type(question, group_col=None, numeric_col=None, dataframe=None):
    """Choose the most useful visualization for the user's question."""
    q = normalize(question)

    if any(word in q for word in ["percentage", "percent", "share", "proportion", "distribution"]):
        return "pie"

    if any(word in q for word in ["relationship", "relation", "correlation", "correlated", "versus", "vs"]):
        if dataframe is not None and len(dataframe.select_dtypes(include="number").columns) >= 2:
            return "scatter"

    if any(word in q for word in ["trend", "over time", "by month", "by year", "monthly", "yearly", "daily", "weekly", "history"]):
        return "line"

    if any(word in q for word in ["compare", "comparison", "highest", "lowest", "top", "bottom", "best", "worst", "most", "least", "which", "by", "group"]):
        return "bar"

    return "bar" if group_col is not None else "metric"


def render_dynamic_chart(question, data, label_col, value_col, dataframe=None):
    """Render a chart chosen from the question and return its chart type."""
    chart_type = choose_chart_type(question, label_col, value_col, dataframe)
    ...
    if chart_type == "pie":
        ...
        ax.set_title(f"{value_col} distribution by {label_col}", color="white", pad=16)
        ...
        ax.set_title(f"{value_col} trend by {label_col}", color="white", pad=16)
        ax.tick_params(axis="x", rotation=35, colors="#cbd5e1")
        ax.tick_params(axis="y", colors="#cbd5e1")
        ax.grid(alpha=0.15)

    elif chart_type == "scatter" and dataframe is not None:
        numeric_cols = dataframe.select_dtypes(include="number").columns.tolist()
        if len(numeric_cols) >= 2:
            ax.scatter(dataframe[numeric_cols[0]], dataframe[numeric_cols[1]], alpha=0.75, s=55)
            ax.set_xlabel(numeric_cols[0], color="#cbd5e1")
            ax.set_ylabel(numeric_cols[1], color="#cbd5e1")
            ax.set_title(f"{numeric_cols[0]} vs {numeric_cols[1]}", color="white", pad=16)
            ax.grid(alpha=0.15)
        else:
            chart_type = "bar"

    if chart_type == "bar":
        plot_data = data.sort_values(value_col, ascending=False)
        ax.bar(plot_data[label_col].astype(str), plot_data[value_col])
        ax.set_title(f"{value_col} by {label_col}", color="white", pad=16)
        ax.tick_params(axis="x", rotation=35, colors="#cbd5e1")
        ax.tick_params(axis="y", colors="#cbd5e1")
...
if question:
        ...
        if (
            ...
            chart_data = grouped.reset_index(name=f"Total {numeric_column}")
            chart_type = render_dynamic_chart(
                question, chart_data, group_column, f"Total {numeric_column}", df
            )
        ...
        # --------------------------------------------------
            ...
            if filter_value is not None:
                ...
                count_data = grouped_count.reset_index(name="Count")
                chart_type = render_dynamic_chart(question, count_data, group_column, "Count", df)
        ...
        # --------------------------------------------------
            ...
            chart_data = grouped.reset_index(name=f"Total {numeric_column}")
            chart_type = render_dynamic_chart(question, chart_data, group_column, f"Total {numeric_column}", df)
        ...
        # PDF REPORT
        if (
            st.session_state.get("pdf_answer")
            and st.session_state.get("pdf_explanation")
        ):
            pdf_bytes = create_pdf_report(
                "Uploaded Dataset",
                question,
                st.session_state["pdf_answer"],
                st.session_state["pdf_explanation"]
            )

            st.download_button(
                "📄 Download PDF Report",
                data=pdf_bytes,
                file_name="DataLens_AI_Report.pdf",
